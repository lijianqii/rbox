...
集成测试在 `tests/run_tests.sh` 中，通过 QEMU 全系统模拟运行所有命令。共 30 个测试组、122 个断言（涵盖 32 个 applet、Shell 全功能、init 服务管理、rgetty/rlogin 登录流程、重启/关机流程）：
...
| 内存信息 | meminfo 输出 Mem/Swap 行、详细明细、iomem 树、进程 VSZ/RSS/%MEM 列表 | 10 |
| **合计** | | **122** |
...
集成测试共 30 个测试组、122 个断言，覆盖全部 32 个 applet 及 Shell/init/重启/关机流程，
