...
| 33 | processes | processes | 以树状显示所有进程及其父子关系（类似 pstree） |
## Shell
...
集成测试在 `tests/run_tests.sh` 中，通过 QEMU 全系统模拟运行所有命令。共 30 个测试组、124 个断言（涵盖 33 个 applet、Shell 全功能、init 服务管理、rgetty/rlogin 登录流程、重启/关机流程）：
...
| 内存信息/进程树 | meminfo 输出、iomem 树、processes 进程树 | 12 |
| **合计** | | **124** |
...
集成测试共 30 个测试组、124 个断言，覆盖全部 33 个 applet 及 Shell/init/重启/关机流程，
...
| sys/* | sleep 6、uname 5、env 4、date 2、true 1、false 1、pwd 1、meminfo 15、proc 4、processes 6 | 46 |
