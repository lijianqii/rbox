...
| 32 | meminfo | meminfo [-bkmg] | 查看内存使用情况（读 /proc/meminfo，类似 free） |
## Shell
...
集成测试在 `tests/run_tests.sh` 中，通过 QEMU 全系统模拟运行所有命令。共 30 个测试组、115 个断言（涵盖 32 个 applet、Shell 全功能、init 服务管理、rgetty/rlogin 登录流程、重启/关机流程）：
...
集成测试共 30 个测试组、115 个断言，覆盖全部 32 个 applet 及 Shell/init/重启/关机流程，
...
| sys/* | sleep 6、uname 5、env 4、date 2、true 1、false 1、pwd 1、meminfo 7 | 27 |
